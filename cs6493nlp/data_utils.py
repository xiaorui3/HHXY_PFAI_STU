...
def make_vocab_from_squad(output_file, counter, max_vocab_size):
    ...
    with open(output_file, "wb") as f:
        pickle.dump(word2idx, f)
    print('word2idx',len(word2idx)) #35322
...
def make_embedding(embedding_file, output_file, word2idx):
    ...
    word2embedding[UNK_TOKEN] = np.zeros(300,)
    ...
    # print('word2embedding',word2embedding[','].shape)
...
def process_txt_files(src_file, tgt_file, ans_file):
    ...
    with open(src_file, "r") as src_f, open(tgt_file, "r") as tgt_f, open(ans_file, "r") as ans_f:
        for src_line, tgt_line, ans_line in tqdm(zip(src_f, tgt_f, ans_f)):
            ...
            for token in ques_tokens:
                counter[token] += len(ques)

            y1s, y2s = [], []
            answer_texts = []
            # print(ans_line)
            
            # for answer_text in ans_line.strip():

            answer_text = ans_line.replace("''", '" ').replace("``", '" ').strip().lower()
            
            answer_start = context.find(answer_text)
            # print('answerstart',answer_start)
            if answer_start < 0:
